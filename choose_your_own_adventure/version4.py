...
This version leans into richer structure rather than simplicity.

Key ideas
---------
- The game is mode-driven and routes between engines.
- The player is not special in combat terms; both player and NPCs are Characters
  with Controllers.
- NPC presets use inheritance mainly to set defaults, while the shared NPC
  class does most of the real work.
- Exits can have attempt / blocked / success effects, so movement itself is
  gameplay.
- Features can be scripted by composing effects.
- Encounters can happen when exploration starts in an area.
- Surrender, bounty, guards, jail, gold, trading, and random encounters all
  emerge from the same underlying structure.
"""
